...
class BookingController extends Controller
{
    ...
    public function visitorAction(Request $request)
    {
        ...
        return $this->render('OCBookingBundle:Visitor:visitors.html.twig', [
            'form'       => $form->createView(),
    ...
    public function paymentAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($stripe_error) {
                $stripe_error = $this->get('translator')->trans($stripe_error, [], 'messages');
            } else {
                return $this->redirectToRoute('oc_booking_checkout');
            }
        }

        return $this->render('OCBookingBundle:Payment:payment.html.twig', [
            'form'         => $form->createView(),
            'prettyDate'   => $this->container->get('oc.bookingbundle.utils')->getPrettyDate($ticket->getVisit()->format('y-m-d')),
            'ticket'       => $ticket,
